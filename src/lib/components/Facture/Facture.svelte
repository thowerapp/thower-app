<script lang="ts">
	...
	async function generateInvoicePDF() {
		...
		autoTable(doc, {
			...
			body: transaction.products.map((product: { name: string; price: number; quantity: number }) => [
				product.name,
		...
		// 💰 Totaux bien espacés
		// ------------------------------
		const finalY = (doc as unknown as { autoTable: { previous: { finalY: number } } }).autoTable.previous.finalY + 15;
		doc.setFontSize(12);
