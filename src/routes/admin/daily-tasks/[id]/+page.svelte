...
<div class="mx-auto max-w-xl px-4 py-8">
	<div class="mb-6 flex items-center gap-3">
		...
		<div>
			...
			<p class="text-sm text-muted-foreground truncate max-w-xs">{data.task.label}</p>
		</div>
	</div>
	...
	<div class="mt-3 flex items-center justify-between">
		<form method="POST" action="?/toggleActive">
			<Button
				type="submit"
				variant="outline"
				class={data.task.active ? 'border-destructive text-destructive hover:bg-destructive/10' : 'border-green-600 text-green-700 hover:bg-green-50'}
			>
				{data.task.active ? 'Désactiver la tâche' : 'Activer la tâche'}
			</Button>
		</form>

		{#if deleteConfirm}
			<form method="POST" action="?/delete" class="flex items-center gap-2">
				<span class="text-sm text-destructive">Supprimer définitivement ?</span>
				<Button type="submit" variant="destructive" size="sm">Confirmer</Button>
				<Button type="button" variant="outline" size="sm" onclick={() => (deleteConfirm = false)}>
					Annuler
				</Button>
			</form>
		{:else}
