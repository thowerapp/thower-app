...
<div class="mx-auto max-w-3xl px-4 py-8">
	<div class="mb-6 flex items-center gap-3">
		...
		<div>
			<h1 class="text-2xl font-bold">Nouvelle recette catalogue</h1>
			<p class="text-sm text-muted-foreground">
				Enregistrée en <span class="font-medium">isCustom : false</span> — visible par tous les clients.
			</p>
		</div>
	</div>

	<form method="POST" action="?/createRecipe" use:enhance class="space-y-6">

		<Form.Field name="name" form={recipeForm}>
			<Form.Control>
				<Form.Label>Nom *</Form.Label>
				<Input name="name" bind:value={$form.name} placeholder="Bowl Bœuf Gourmand…" />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
		...
		<Form.Field name="description" form={recipeForm}>
			<Form.Control>
				...
				<Textarea name="description" bind:value={$form.description as string} rows={3} placeholder="Accroche courte de la recette…" />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
		...
		<div class="grid grid-cols-3 gap-4">
			...
			<Form.Field name="totalTimeMin" form={recipeForm}>
				<Form.Control>
					...
					<Input name="totalTimeMin" type="number" min="0" bind:value={$form.totalTimeMin as number} placeholder="25" />
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
		...
		<Form.Field name="referenceYieldG" form={recipeForm}>
			<Form.Control>
				...
				<Input name="referenceYieldG" type="number" min="0" bind:value={$form.referenceYieldG as number} placeholder="Ex : 630" />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
		...
		<Form.Field name="instructions" form={recipeForm}>
			<Form.Control>
				...
				<Textarea name="instructions" bind:value={$form.instructions as string} rows={8} placeholder="Étapes de la recette…" />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
				...
				<div class="mb-4 rounded-lg border p-4 relative">
					...
					<div class="grid grid-cols-3 gap-3">
						<div class="flex flex-col gap-1">
							...
							<Input id={`ing-name-${i}`} name={`ingredients[${i}].name`} bind:value={ingredients[i].name} placeholder="Steak haché…" />
						</div>

						<div class="flex flex-col gap-1">
							<label for={`ing-qty-${i}`} class="text-sm font-medium">Qté (g)</label>
							<Input id={`ing-qty-${i}`} name={`ingredients[${i}].quantityG`} type="number" min="0" bind:value={ingredients[i].quantityG as number} placeholder="200" />
						</div>

						<div class="flex flex-col gap-1">
							<label for={`ing-unit-${i}`} class="text-sm font-medium">Unité</label>
							<Input id={`ing-unit-${i}`} name={`ingredients[${i}].unit`} bind:value={ingredients[i].unit as string} placeholder="g, ml, c.à.s…" />
						</div>

						<div class="flex flex-col gap-1">
							<label for={`ing-cat-${i}`} class="text-sm font-medium">Catégorie courses</label>
							<Input id={`ing-cat-${i}`} name={`ingredients[${i}].category`} bind:value={ingredients[i].category as string} placeholder="Viandes, Légumes…" />
						</div>

						<div class="flex flex-col gap-1 col-span-2">
							<label for={`ing-note-${i}`} class="text-sm font-medium">Note</label>
							<Input id={`ing-note-${i}`} name={`ingredients[${i}].note`} bind:value={ingredients[i].note as string} placeholder="Ex. facultatif, +60 kcal…" />
						</div>
					</div>
