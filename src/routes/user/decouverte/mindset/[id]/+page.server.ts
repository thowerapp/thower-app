...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	if (!db?.discoveryContent) throw redirect(302, '/user/decouverte/mindset');
	...
	if (!content || !content.active || content.category !== 'MINDSET') {
		throw redirect(302, '/user/decouverte/mindset');
	}
			...
			if (result.completedNow) {
				...
				await (prisma as any).pointEvent?.create({
					data: {
						...
						type: 'VIDEO_WATCHED',
						amount: 10,
						metadata: { discoveryContentId: id, source: 'mindset' }
					}
				});
